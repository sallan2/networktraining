...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{grouped <-}\StringTok{ }\NormalTok{iris }\OperatorTok{%>%}\StringTok{ }\KeywordTok{group_by}\NormalTok{(Species) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{mean_petal_length =} \KeywordTok{mean}\NormalTok{(Petal.Length))}
\NormalTok{grouped}
\end{Highlighting}
\end{Shaded}
...
This is a Web Exercise template created by the \href{http://www.psy.gla.ac.uk}{psychology teaching team at the University of Glasgow}, based on ideas from \href{https://software-carpentry.org/lessons/}{Software Carpentry}. This template shows how instructors can easily create interactive web documents that students can use in self-guided learning.

The webex package provides a number of functions that you use in \href{https://github.com/rstudio/cheatsheets/raw/master/rmarkdown-2.0.pdf}{inline R code} to create HTML widgets (text boxes, pull down menus, buttons that reveal hidden content). Examples are given below. Knit this file to HTML to see how it works.

\textbf{NOTE: To use the widgets in the compiled HTML file, you need to have a JavaScript-enabled browser. The widgets don't work in the built-in RStudio browser. In the built-in browser, click the ``Open in Browser'' button to open the file in your operating system's browser.}

\hypertarget{fill-in-the-blanks-fitb}{%
\section{\texorpdfstring{Fill-In-The-Blanks (\texttt{fitb()})}{Fill-In-The-Blanks (fitb())}}\label{fill-in-the-blanks-fitb}}

Create fill-in-the-blank questions using \texttt{fitb()}, providing the answer as the first argument.
...
The blanks are case-sensitive; if you don't care about case, use the argument \texttt{ignore\_case\ =\ TRUE}.
...
If you want to ignore differences in whitespace use, use the argument \texttt{ignore\_ws\ =\ TRUE} (which is the default) and include spaces in your answer anywhere they could be acceptable.
...
\hypertarget{multiple-choice-mcq}{%
\section{\texorpdfstring{Multiple Choice (\texttt{mcq()})}{Multiple Choice (mcq())}}\label{multiple-choice-mcq}}
...
\item
  ``I bless the rains guess it rains sense the rain down in Africa'' -Toto
\end{itemize}

\hypertarget{true-or-false-torf}{%
\section{\texorpdfstring{True or False (\texttt{torf()})}{True or False (torf())}}\label{true-or-false-torf}}
...
\hypertarget{hidden-solutions-and-hints}{%
\section{Hidden solutions and hints}\label{hidden-solutions-and-hints}}

You can fence off a solution area that will be hidden behind a button using \texttt{hide()} before the solution and \texttt{unhide()} after, each as inline R code. Pass the text you want to appear on the button to the \texttt{hide()} function.

If the solution is an RMarkdown code chunk, instead of using \texttt{hide()} and \texttt{unhide()}, simply set the \texttt{webex.hide} chunk option to TRUE, or set it to the string you wish to display on the button.

\hypertarget{example-problem}{%
\subsection{Example problem}\label{example-problem}}

\textbf{Recreate the scatterplot below, using the built-in \texttt{cars} dataset.}
